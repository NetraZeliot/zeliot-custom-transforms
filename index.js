      ...
      try{
        ...
        let redisKey = `imei_${vehicle}`;
          ...
          if (redisValue) {
            ...
            if (redisValue === "battery_disconnect_start") {
              console.log(`Key '${redisKey}' has value 'battery_disconnect_start'. No action needed.`);
            } else {
              let messageToPublish = JSON.stringify({ imei: vehicle, status: "battery_disconnect_stop" });
              await producer.send({
            ...
            if ((eventFlag & 128) === 128) {
              await writeToRedisWithKeyValue(redisKey, "battery_disconnect_start");
              console.log(`Stored IMEI ${vehicle} with value 'battery_disconnect_start' in Redis.`);
  
              let messageToPublish = JSON.stringify({ imei: vehicle, status: "battery_disconnect_start" });
              await producer.send({
                topic: process.env.KAFKA_PRODUCER_TOPIC,
                messages: [{ value: messageToPublish }],
              });
