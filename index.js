      ...
      try{
        ...
        let redisKey = `imei_${vehicle}`;
          ...
          if (redisValue) {
            ...
            if (redisValue === "harsh_cornering_start") {
              console.log(`Key '${redisKey}' has value 'harsh_cornering_start'. No action needed.`);
            } else {
              let messageToPublish = JSON.stringify({ imei: vehicle, status: "harsh_cornering_stop" });
              await producer.send({
            ...
            if ((eventFlag & 268435456) === 268435456) {
              await writeToRedisWithKeyValue(redisKey, "harsh_cornering_start");
              console.log(`Stored IMEI ${vehicle} with value 'harsh_cornering_start' in Redis.`);
  
              let messageToPublish = JSON.stringify({ imei: vehicle, status: "harsh_cornering_start" });
              await producer.send({
                topic: process.env.KAFKA_PRODUCER_TOPIC,
                messages: [{ value: messageToPublish }],
              });
